#!/bin/bash
    ...
    #   GeoTiffs with JPEG compression don't work with the gdalgeotiff & turbojpeg 
    #   ECW and MrSID artifacts are excluded from general GDAL install and explicitly added layer in the script
    PROBLEM_ARTIFACTS="imageio-ext-turbojpeg-${IMAGEIO_EXT_VER}.jar imageio-ext-gdalgeotiff-${IMAGEIO_EXT_VER}.jar"  # don't install without libjpeg-turbo
...
#                unzip -o $GDAL_PLUGIN_ZIP -x $ECW_ARTIFACTS $MRSID_ARTIFACTS -d $GEOSERVER_LIB_PATH
                ...
                # Copy remaining jars to GeoServer (don't overwrite)
                cp -n *.jar $GEOSERVER_LIB_PATH
                # Clean up
                popd
                rm -rf $TEMP_DIR
...
#                unzip -o $IMAGEIO_EXT_ZIP -x $ECW_ARTIFACTS $MRSID_ARTIFACTS  -d $GEOSERVER_LIB_PATH
                ...
                # Delete the proprietary and jars
                for file in $IGNORED_ARTIFACTS; do rm $file; done
                # Copy remaining jars to GeoServer (don't overwrite)
                cp -n *.jar $GEOSERVER_LIB_PATH
                # Clean up
