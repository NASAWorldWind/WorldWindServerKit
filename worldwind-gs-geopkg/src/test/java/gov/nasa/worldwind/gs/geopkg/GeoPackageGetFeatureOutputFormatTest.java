...
public class GeoPackageGetFeatureOutputFormatTest extends WFSTestSupport {
    ...
    public void init() {
        ...
        oldLevel = Logging.getLogger(SqlUtil.class).getLevel();
        Logging.getLogger(SqlUtil.class).setLevel(Level.FINEST);
    }

    @After
    public void restore() {
        Logging.getLogger(SqlUtil.class).setLevel(oldLevel);
    }
    ...
    // Bad test data?
    @Ignore (" java.sql.SQLException: [SQLITE_CONSTRAINT]  Abort due to constraint violation (UNIQUE constraint failed: gpkg_geometry_columns.table_name)")
    @Test
    public void testGetFeatureTwoTypes() throws IOException {
        FeatureCollectionResponse fct
                = FeatureCollectionResponse.adapt(WfsFactory.eINSTANCE.createFeatureCollectionType());

        // Create the first feature type
        FeatureSource<? extends FeatureType, ? extends Feature> fs;
        fs = getFeatureSource(SystemTestData.SEVEN);;
        fct.getFeature().add(fs.getFeatures());

        // Create the second feature type
